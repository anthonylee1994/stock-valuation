        ...
        <div>
            <div className="relative h-2 rounded-full overflow-visible">
                <div className="absolute inset-0 flex rounded-full overflow-hidden">
                    <div className={`h-full bg-success ${barTransition}`} style={{width: `${lowPosition}%`}} />
                    <div className={`h-full bg-warning ${barTransition}`} style={{width: `${highPosition - lowPosition}%`}} />
                    <div className={`h-full bg-danger flex-1 ${barTransition}`} />
                </div>
                <Tooltip delay={0} isOpen={isTooltipOpen} onOpenChange={setIsTooltipOpen}>
                    <div className="absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center transition-[left] duration-500 ease-in-out" style={{left: `${markerPosition}%`}}>
                        <Tooltip.Trigger onClick={() => setIsTooltipOpen(!isTooltipOpen)} className="absolute -top-0.5 -translate-y-1/2 flex flex-col items-center">
                            ...
                            <div className="w-1 h-5 bg-foreground -mt-1.5 rounded-2xl" />
                        </Tooltip.Trigger>
                    </div>
